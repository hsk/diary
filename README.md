...
LaTeXiTっていうツールがまず便利です。ライブラリにスニペット的に様々な数式を保存しておく事が出来ます。MathJaxなどでは数式しかあつかえませんが、マクロを使う事も出来るので様々な制御が出来ます。代わりにコンパイル時間がかかるのですが、ライブラリに画像として保存する事が出来るので、一覧を見てさっと取り出して参考にする事が出来ます。ぱっとみてさっと取り出せるのが素晴しいのです。
...
龍田先生のproof.sty やbussproof.styがあります。
...
HTMLの世界を考えてみましょう。HTMLはTeXより後発のもので、組版を意識していません。段組やページの概念がありません。それ故に高速に動作します。テーブルレイアウトやCSSのスタイルシートによって、位置調整も楽に出来ます。JavaScriptはHTML上のマクロ言語のようなものでプログラムで様々な事を実現出来ます。CSSファイルは便利で色々な人が作って奇麗な見た目を静的に実現出来ます。よりたくさんのJavaScriptやCSSが乱立し、ブラウザの種類が増えて行き、何処でもしっかり動くものが必要になりました。jquery.jsやbootstrap.cssは標準的で便利なライブラリとして提供されています。

C++にはstlがあるけどやっぱり駄目だboostのようなライブラリが必要そうだという感じです。

TeXにはパッケージシステムもないようです。npmのようなパッケージマネージャがあればもっと便利になるでしょう。
